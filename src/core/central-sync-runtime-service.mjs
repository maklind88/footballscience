export function createCentralSyncRuntimeService(deps = {}) {
  ...
  function isSameCentralManifestGeneration(first = null, second = null) {
    if (!first || !second) {
      return false;
    }
    return (
      first.principalScope === second.principalScope &&
  ...
  function setCentralSyncPendingState(key, isPending = false, isRemoved = false, principalScope = "", options = {}) {
    ...
    const mutation = mutateManifestWithResult((manifest) => {
      ...
      manifest.entries[normalizedKey] = {
        ...
        deletedAt: isPending
          ? (isRemoved ? getDataSafetyNow() : "")
          : (isRemoved ? currentEntry.deletedAt || getDataSafetyNow() : currentEntry.deletedAt || ""),
      };
      ...
      if (isPending) {
        generation = createCentralManifestGeneration(
          manifest.entries[normalizedKey],
          isRemoved ? null : (typeof options.value === "string" ? options.value : rawGetItem(normalizedKey))
        );
      }
    });
  ...
  function acknowledgeCentralStateWrite(write = {}) {
    ...
    if (queuedWrite && queuedWrite.principalScope !== write.principalScope) {
      return false;
    }
    if (
      queuedWrite &&
      Number(queuedWrite.generation) > Number(write.generation) &&
      !isSameCentralManifestGeneration(queuedWrite.manifestGeneration, write.manifestGeneration)
    ) {
      return false;
    }
    if (
      ...
      centralStateWriteQueue.delete(String(write.key || ""));
    }
  ...
  function queueCentralStateWrite(key, value, options = {}) {
    ...
    if (isCentralStateTransitionFenceActive()) {
      setCentralSyncPendingState(normalizedKey, false, Boolean(options.removed), principalScope, {
        ...
        expectedValue: options.removed ? null : textValue,
      });
      queueCentralStateStatus("Local save is paused while account data is being isolated safely.");
      reportSyncStatus(normalizedKey, "issue", "Save paused");
      return false;
