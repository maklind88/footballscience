...
When a daily report is red because of a long-running query or an exclusive lock, run the workflow manually with `investigate` enabled. That mode adds one fixed read-only query under `default_transaction_read_only=on` and stores only privacy-safe categories, age buckets, booleans, and short one-way fingerprints so repeated signals can be correlated. It intentionally omits raw SQL, relation names, process IDs, transaction IDs, application names, user names, and database values, and it still makes no database changes.
