...
- `data`: database-primary metadata in `video_matches`, `video_videos`, `video_sources`, `video_clip_instances`, `video_clip_players`, `video_clip_tags`, `video_clip_notes`, `video_coding_schemas`, `video_playlists`, `video_playlist_items`, `video_coding_templates`, `video_coding_buttons`, `video_coding_button_links`, `video_clip_labels`, `video_clip_descriptors`, `video_timeline_lanes`, `video_saved_clip_searches`, `video_playlist_sections`, `video_review_sessions`, `video_clip_revisions`, `video_object_tracks`, `video_track_corrections`, `video_dynamic_graphics`, `video_pitch_calibrations`, and `video_pitch_calibration_frames`; dense tracking samples and raw media remain local, with no protected browser storage key and no cloud video uploads.
...
- `qa`: schema, API payload safety, module boundary, workstation UX, permission matrix, tracking review/correction, secure local provider jobs, pitch calibration, metre confidence gates, and Analysis Room loader contracts must prove metadata-only central storage, millisecond precision, no component database access, keyboard-ready coding, no fabricated movement continuity, and no local video path leakage.
