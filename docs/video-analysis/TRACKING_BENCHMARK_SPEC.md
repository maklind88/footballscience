...
`--trackeval` is deliberately explicit. Without it, a multi-object report remains `providerApprovalReady: false`. With it, the report contains only bounded metric evidence and hashes, never source paths, raw tracks, frames, or media. Real provider approval still requires human-verified real-match cases and reviewed model/data provenance. Measurement Intelligence may derive a read-only diagnostic queue only when every case references the same verified report and has complete TrackEval metrics for player, ball, and referee. The queue sorts signed target margins across case, entity, and metric; these per-entity targets guide investigation but do not add approval gates. Each hotspot exposes its measured recommendation and may reconnect the original review case only when the evaluation evidence set, benchmark id, locked artifact, source, range, workload workspace, campaign item, clip and camera angle all resolve to one exact context. The independently recomputed internal benchmark can contribute at most three bounded, identity-free candidate checkpoints per hotspot from per-entity misses, false positives, overlap, identity switches and fragmentations. The UI labels these as internal diagnostics rather than TrackEval timestamps, and a checkpoint seek is accepted only inside the exact saved clip range. A crossed or ambiguous identity keeps the metric visible but removes navigation. The queue remains absent for crossed report identity, failed case cross-validation, or partial entity evidence and cannot change the report or its verdict.
...
Candidate-assisted annotation has its own fail-closed boundary. `tracking:candidate:preannotation` requires one independently verified full-range detection bundle and accepts full-range association evidence only when its sealed review gate explicitly permits a suggestion layer. A failed association screen remains failed and can never approve a provider, but its partial trajectories may seed review while every unassigned observation stays visible. Detection-only workspaces preserve every observation as an explicit unassociated suggestion and require association review; they are preferable to hiding a provider that failed its full-range trajectory bound. The command deterministically exports read-only MOT suggestions and metadata-only track maps to a new directory and binds every output byte to the exact candidate artifact hashes. It never opens or writes the pack's reference annotation files. `tracking:candidate:preannotation:verify` rebuilds the complete workspace from the sealed candidate evidence and fails on a linked, writable, changed, partial, hidden-field, blocked-review-gate, or non-reproducing artifact. The result remains a suggestion source only: exhaustive false-negative search, referee and ball review, player/team/identity assignment, dataset-rights review, and a separately attested MOT import are mandatory before metrics can run.

The in-product preannotation queue keeps that boundary intact. The analyst selects the pack manifest, workspace manifest, and one matching track-map/MOT pair after reconnecting the exact normalized case clip. Browser-side SHA-256, workspace checksum, source, geometry, count, association state, temporal order, and lineage verification runs before review begins. The normal tracking overlay renders only one pending suggestion at a time, prioritizing referee and ball work before player trajectories. Accept and reject decisions are reversible; in a signed-in tracking scope, unresolved decisions and undo history autosave to an isolated local IndexedDB draft and restore only for the exact organization, team, user, source, clip, item, angle, workspace hash, and case. Without that scope the panel states that progress lasts for the browser session. A pending suggestion is a write-protected preview: verification, continuation, correction persistence, and dynamic-graphic binding reject it. `Save & correct` persists only that suggestion as a client-generated `review` track and keeps it selected for the normal identity, position, visibility, continuity, merge, split, identity-swap, undo, and redo tools; `Continue` returns to the queue. Batch `Save accepted` remains available. Changing clips clears an unsaved preview rather than carrying evidence across sources, while an older session's asynchronous storage result cannot modify the new session. Ground-truth preparation additionally requires both the exact review draft and campaign ledger to report a durable device checkpoint. Saving, failed, pending, or session-only progress fails closed; an explicit retry reuses the current source-bound decision state without repeating decisions, while session-only work must be reopened after sign-in. Every restored campaign row is revalidated against the sealed workspace, case id and exact case source before its counts can enter the campaign view; an item/clip/angle resume context must be complete or absent, and one invalid row blocks a new save without deleting its device record. The campaign QA matrix derives each visible case phase from that exact presentation context, unresolved saved roles, source-bound workload evidence, and normalized scene-review checkpoints. Decisions, unresolved roles, missing context, partial scene review, completed scene review, exact locked reference, and a local/suite reference-identity mismatch remain separate fail-visible states. Review Studio counts a campaign reference only when its workspace, case id, locked artifact source, and workload source all match the exact sealed campaign source. After a lock, the next unresolved case creates an explicit source-bound handoff before the local-video picker opens. Once the source fingerprint matches, Review Studio exposes a separate workspace-open action; the controller restores the saved item, clip, and camera angle, then revalidates that exact context before any workspace files are selected or imported. Missing targets, a wrong source, or a changed angle fail closed and leave the handoff intact for correction. The handoff is cleared only after the matching case and source have opened; source hashing and workspace selection still run again and no reference state is copied across cases. During exhaustive scene review, `Review context` plays a bounded 1.5-second window before and after the exact displayed, source-bound checkpoint, clipped to the reference range. It stops without marking the checkpoint reviewed and is cancelled by review, navigation, reset, attestation, draft invalidation, or leaving tracking mode. The analyst must still inspect and explicitly confirm every checkpoint. A draft also requires a named reviewer or analyst ID before it can lock; authenticated identity may prefill that field, known placeholders and control text are rejected in both the browser and MOT import path, and changing the reviewer clears every existing attestation and checkpoint mark. New `football-science-ground-truth-scene-review-v2` ledgers persist the same normalized reviewer with source, angle and range, while older locked v1 evidence remains readable without allowing anonymous in-progress marks to become new evidence.

Before a human review session, `tracking:ground-truth:preflight` combines the existing independent verifiers into one read-only campaign gate. It reproduces the exact preannotation workspace, cross-checks every review sequence against the sealed pack case id, clip SHA-256, source time, frame count, scenario tags and relative source/annotation file, audits annotation progress, and reruns the pinned TrackEval synthetic control. Its metadata-only report distinguishes `ready-for-human-review` from `ready-for-measurement`; an installed evaluator or valid suggestion workspace can never promote empty or unattested annotations.
...
The same exact scope has a separate metadata-only review-effort ledger. It cumulatively counts completed accept, reject, undo, defer, correction-handoff, batch-save, and saved-track actions per sealed case; it does not estimate analyst time from idle browser intervals. Partial batch failures retain only the tracks actually saved, and a retry adds only its newly completed work. Version-one campaign records migrate with `partial` effort coverage so historical interactions are never invented. The campaign panel reports case and aggregate actions per sealed suggestion while keeping effort coverage visible. These counters measure annotation workload only: they cannot attest exhaustive review, change ground truth, approve a provider, or substitute for precision, recall, HOTA, IDF1, identity switches, class accuracy, or TrackEval evidence.

`Use in ground truth` is available only when the active sealed case is resolved, has at least one saved track, and has complete version-two effort coverage. That transition creates one strict `football-science-tracking-review-workload-evidence-v1` snapshot containing only the exact source/workspace/case/angle/range identity, resolved outcomes, raw action counters, and reproducible ratios. Hidden fields, partial history, mismatched saved-track counts at the handoff, changed source/range, or forged summaries fail closed. The snapshot is optional for manually authored or imported ground truth and is forbidden for selected-object references. For a full-scene preannotation case it is retained beside, never inside, the independent real-match attestation. Later track selection and refinement preserve the original snapshot; source, angle, range, or explicit new-draft changes clear it.

Locking embeds the validated workload snapshot in that exact full-scene ground-truth case. The immutable suite export therefore includes it in `groundTruthSuiteSha256`; changing one workload byte changes or invalidates the benchmark evidence binding. Provider evaluation cases intentionally omit the snapshot, so workload can be analysed as annotation burden without becoming a provider metric, threshold, approval claim, or substitute for exhaustive review.

Every newly locked ground-truth revision also carries strict metadata-only `football-science-tracking-annotation-burden-evidence-v1`. It binds the source, angle and range; counts retained preannotations, excluded suggestions and tracks added outside preannotation; records automatic, manual and interpolated samples; deduplicates paired structural corrections into correction operations; and records reviewed checkpoints. Exact-key validation and the suite checksum make later mutation visible. Older imported artifacts remain valid without this optional evidence. Provider evaluation inputs omit it, and no annotation-burden value can approve a provider or replace rights review, exhaustive scene attestation, or independent quality metrics.
...
Locking creates a new revisioned snapshot. Later edits to live tracks do not mutate the locked reference. The artifact contains reviewed normalized trajectories, source fingerprint, frame/range, object identity, bounded analyst evidence, the refinement-burden summary, and the optional independently validated workload snapshot. Provider metadata, confidence values, correction authors, local paths, URLs, video, frames, and binary data are removed. Ground truth is not written through the central tracking repository.
...
3. Association fuses detections into continuity trajectories and exposes discontinuities.
4. Re-identification maintains an opaque local identity through occlusion and cuts.
5. Team and shirt classification add confidence-gated identity evidence.
...
An approved future stage is discoverable only through the local `football-science-tracking-provider-registry-v1` boundary. Its installation marker binds the exact canonical manifest, original report, reproducible evidence, sealed runtime, and every model artifact by relative path, byte count, and SHA-256. Registry discovery rejects links, path escape, duplicate provider ids, changed artifacts, and self-consistently rehashed reports whose evidence no longer reproduces. The browser receives bounded public capability metadata only; registry readiness is not permission to execute the provider. `football-science-tracking-stage-execution-v1` separately requires one native sealed runtime, a successful network-denied sandbox preflight, exact session-owned source SHA-256, provider concurrency, output, wall-time and process-group memory limits, cancellation, and final request-bound result validation. Source, runtime, and model files are read-only and bind both content and descriptor identity before execution; descriptor drift after execution rejects the output. If any execution condition is unavailable, the provider remains `activation pending` even when its benchmark evidence is valid.
...
The browser seals the four raw stage runs, composite lineage, synchronized source and match ranges, raw review tracks, and review summary into one `football-science-tracking-candidate-pipeline-run-v1` artifact before any review track is published. Full artifacts are retained only in an exact organization/team/user/source/item IndexedDB scope; the list view reads separately bounded summaries, while restore revalidates every stage checksum and the composite content hash. Storage is fail-closed at 12 runs per item, 48 runs or 512 MiB per scope and never silently evicts evidence. Export and explicit removal remain available from the Tracking Intelligence panel.

One pipeline produces four independent `football-science-tracking-provider-run-v1` artifacts. Detection, association, re-identification, and classification keep their own provider build, capabilities, execution fingerprint, prediction tracks, and complete workstation profile. The analyst explicitly selects which stage provider is bound to the full-scene benchmark; changing that stage invalidates any prior evaluation. Candidate selection affects benchmark evidence only and never replaces the activated provider used by normal tracking.
