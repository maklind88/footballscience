...
const APPLY_RPC = "apply_platform_app_state_tenant_migration";
const ROLLBACK_RPC = "rollback_platform_app_state_tenant_migration";
...
async function updateJournal(config, id, values, guards = {}, fetchImpl = fetch) {
  const params = { id: `eq.${id}`, select: "id,status,plan_sha256" };
  if (guards.expectedStatus) params.status = `eq.${guards.expectedStatus}`;
  if (guards.expectedPlanSha256) params.plan_sha256 = `eq.${guards.expectedPlanSha256}`;
  const result = await request(restUrl(config, JOURNAL_TABLE, params), {
    method: "PATCH",
    headers: headers(config.serviceRoleKey, { Prefer: "return=representation" }),
  ...
  return rows.length === 1 && rows[0]?.id === id
    ? { ok: true, row: rows[0] }
    : { ok: false, status: 409, reason: "Migration journal CAS update affected no rows." };
}

async function applyTargetRowsTransactionally(config, journalId, plan, actorId, expectedJournalStatus, fetchImpl = fetch) {
  const result = await request(`${config.url}/rest/v1/rpc/${APPLY_RPC}`, {
    method: "POST",
    headers: headers(config.serviceRoleKey),
    body: JSON.stringify({
      p_migration_id: journalId,
      p_target_organization_id: plan.targetOrganizationId,
      p_plan_sha256: plan.planSha256,
      p_actor_id: actorId,
      p_source_rows: plan.source.rows.map((row) => ({
        ...row,
        valueHash: SHA256_PATTERN.test(row.valueHash) ? row.valueHash : sha256(row.value),
      })),
      p_expected_record_count: plan.source.recordCount,
      p_expected_source_content_sha256: plan.source.contentSha256,
      p_expected_source_revision_sha256: plan.source.revisionSha256,
      p_expected_target_before_record_count: plan.targetBefore.recordCount,
      p_expected_journal_status: expectedJournalStatus,
    }),
  }, fetchImpl);
  const applied = Array.isArray(result.payload) ? result.payload[0] : result.payload;
  if (!result.ok || applied?.applied !== true || Number(applied?.migrated_count) !== plan.source.recordCount) {
    throw new Error(result.reason || "Legacy app-state rows could not be migrated transactionally.");
  }
}

async function rollbackTargetRowsTransactionally(config, row, snapshot, fetchImpl = fetch) {
  const result = await request(`${config.url}/rest/v1/rpc/${ROLLBACK_RPC}`, {
    method: "POST",
    headers: headers(config.serviceRoleKey),
    body: JSON.stringify({
      p_migration_id: row.id,
      p_plan_sha256: row.plan_sha256,
      p_source_rows: snapshot.source.rows,
      p_expected_record_count: snapshot.source.recordCount,
      p_expected_source_content_sha256: row.source_content_sha256,
      p_expected_source_revision_sha256: row.source_revision_sha256,
      p_expected_journal_status: row.status,
    }),
  }, fetchImpl);
  const rolledBack = Array.isArray(result.payload) ? result.payload[0] : result.payload;
  if (!result.ok || rolledBack?.rolled_back !== true || Number(rolledBack?.deleted_count) !== Number(snapshot.source.recordCount)) {
    throw new Error(result.reason || "Target rollback could not be completed transactionally.");
  }
}
...
async function rollbackMigration(config, options, fetchImpl = fetch) {
  ...
  if (
    ...
    !SHA256_PATTERN.test(String(row.source_content_sha256 || "")) ||
    !SHA256_PATTERN.test(String(row.source_revision_sha256 || "")) ||
    !["applying", "failed", "completed"].includes(String(row.status || ""))
  ) {
    throw new Error("Rollback journal is missing the required CAS plan/hash/status fields.");
  }
  ...
  await rollbackTargetRowsTransactionally(config, row, storedSnapshot.snapshot, fetchImpl);
...
export async function runAppStateTenantMigration(options, dependencies = {}) {
  ...
  if (resumableJournal) {
    const resumed = await updateJournal(config, journal.row.id, {
      status: "applying",
      failed_at: null,
      last_error: "",
    }, {
      expectedStatus: resumableJournal.status,
      expectedPlanSha256: plan.planSha256,
    }, fetchImpl);
    if (!resumed.ok) throw new Error(resumed.reason || "Migration journal could not be marked resumable.");
  }
  try {
    await applyTargetRowsTransactionally(config, journal.row.id, plan, options.actorId, "applying", fetchImpl);
    ...
    const completed = await updateJournal(config, journal.row.id, {
      status: "completed",
    ...
    if (!completed.ok) {
      throw new Error(`Migration journal could not be marked completed: ${completed.reason || "unknown journal error"}`);
    }
    return {
      ...summary,
      dryRun: false,
    ...
    const failed = await updateJournal(config, journal.row.id, {
      status: "failed",
      ...
      expectedStatus: "applying",
      expectedPlanSha256: plan.planSha256,
    }, fetchImpl).catch((journalError) => ({ ok: false, reason: journalError?.message || "Migration journal failure status could not be written." }));
    if (!failed.ok) {
      throw new Error(`${error.message} Migration journal failure status was not persisted: ${failed.reason || "unknown journal error"}. No automatic rollback was attempted; use the explicit verified rollback command.`);
    }
