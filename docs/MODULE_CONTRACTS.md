...
- `qa`: periodization day notes persist after refresh; `qa/periodization-module-contract.api.spec.mjs` locks the extracted state, renderer, controller bindings, and merge boundary.
- `migration`: current app-state remains active while state normalization, stale-tab merge helpers, rendering, and workspace controller bindings live in `src/modules/periodization`. Move to `periodization_days` after schedule and session foundations are stable; cross-module Session Planner overlay wiring remains in `app.js` until the next extraction pass.
