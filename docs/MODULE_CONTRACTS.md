...
- `qa`: schema, API payload safety, module boundary, workstation UX, permission matrix, tracking review/correction, secure local provider jobs, and Analysis Room loader contracts must prove metadata-only central storage, millisecond precision, no component database access, keyboard-ready coding, and no local video path leakage.
