...
- Vercel release traffic guard through `npm run release:traffic`, so local deploy tooling stops when staging deploy, production deploy, or rollback is already active.
