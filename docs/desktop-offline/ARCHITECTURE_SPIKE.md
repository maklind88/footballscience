...
The packaged Candidate A shell renders and edits the selected projection through exactly two typed operations: session rename and block-duration change. SQLite schema v3 stores the bounded projection plus an atomic outbox, durable acknowledgement receipts and a sidecar quarantine that preserves unauthorized pending work while excluding it from resend. A bounded status command exposes only synced/pending/blocked/revoked state and counts. Contract tests cover UI envelope construction and state presentation, typed native operations, transaction rollback, close/reopen persistence, accepted-response loss, idempotent replay, acknowledgement-before-delete, expired leases and unauthorized partition/revocation behavior. Candidate B and recovery remain non-mutating.
