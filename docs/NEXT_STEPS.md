...
- Less-is-more Gameplan foundation is in place: four visible tabs (`Plan`, `Staff`, `Player Brief`, `Matchday`), a read-first Match Command briefing with separate Edit mode, starting XI/bench selection in overview, week-focus principles synced from Periodization/Session Planner, mini-game principle player assignment, Staff role lens for responsibilities/evidence/keeper/player-safe delivery, top three decision triggers, evidence chips linked to Analysis Room/Scouting/match media sources, staff meeting approval, matchday coach mode, selected-player Player Brief publishing, signed Player Brief links, token-gated player portal payloads, and opened/acknowledged receipts.
- Next pass: add reminder/notification flow for unread player briefs, link-expiry controls, stronger multi-player ownership for mini-game principles, and durable clip/media IDs when Analysis Room graduates from placeholder to real video workflow.
