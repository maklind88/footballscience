...
- Less-is-more Gameplan foundation is in place: four visible tabs (`Plan`, `Staff`, `Player Brief`, `Matchday`), a read-first Match Command briefing with separate Edit mode, top three decision triggers and evidence chips, staff meeting approval, matchday coach mode, selected-player Player Brief publishing, signed Player Brief links, token-gated player portal payloads, and opened/acknowledged receipts.
