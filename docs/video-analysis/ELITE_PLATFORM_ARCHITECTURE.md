...
- Full-scene candidate execution composes detection, association, re-identification, and classification without weakening their provider boundaries. Complete detection observations are materialized into ordered trajectories before identity stages, every stage contributes evidence/result hashes to one lineage fingerprint, and the resulting player/ball/referee tracks enter normal review. Position, visibility, continuity, split, identity swap, team/shirt, and player assignment corrections operate on review copies while raw candidate stage evidence remains recursively immutable. A source-, workspace-, case-, tenant-, user-, clip-, item-, and angle-bound local effort ledger separately counts exact review decisions, undo/defer actions, correction handoffs, and successful saved tracks. Legacy coverage remains explicitly partial, and workload telemetry has no path to provider approval or ground-truth attestation.
