...
Rust origin tests cover macOS- and Windows-shaped custom origins, reject the Windows HTTPS lookalike, and enforce external-navigation denial. A packaged Apple Silicon macOS build loaded the signed active generation through the custom protocol. [Windows CI run 35932204339](https://github.com/maklind88/footballscience/actions/runs/35932204339) then loaded the current full-platform Candidate A through WebView2's intercepted `http://fs-active.localhost` and `http://fs-candidate.localhost` virtual origins, completed signed activation and IPC, and rejected both the ungranted native command and a command attempted from `http://127.0.0.1:47843`. The runner exposed no privileged localhost listener for these virtual origins.
