...
for (const width of [1388, 390]) {
  test(`camera and settings stay compact, ordered and functional at ${width}px`, async ({ page }, testInfo) => {
    ...
    await settings.click();
    await expect(menu).toBeHidden();
    const playerBox = await page.locator(".video-analysis-player").boundingBox();
    await camera.click();
    await expect(panel).toBeVisible();
    await expect(camera).toHaveAttribute("aria-expanded", "true");
    await expect(camera).toHaveAttribute("aria-haspopup", "dialog");
    expect(await panel.evaluate(el => el.matches(":modal"))).toBe(true);
    expect(await page.locator(".video-analysis-player").boundingBox()).toEqual(playerBox);
    const cameraBox = await panel.boundingBox();
    expect(cameraBox.x).toBeGreaterThanOrEqual(0);
    expect(cameraBox.x + cameraBox.width).toBeLessThanOrEqual(width);
    expect(cameraBox.y + cameraBox.height).toBeLessThanOrEqual(762);
    expect(await panel.evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true);
    await expect(page.getByRole("navigation", { name: "Media production" })).toBeVisible();
    await page.screenshot({ path: testInfo.outputPath(`camera-popup-${width}.png`) });
    await page.getByRole("button", { name: "Close cameras and media" }).click();
    await expect(panel).toBeHidden();
...
test("camera and settings remain reachable in code mode", async ({ page }) => {
  ...
  await expect(settings).toHaveAttribute("aria-expanded", "false");
  await expect(page.locator("[data-video-analysis-fs-player-workstation]")).toHaveClass(/is-code-mode/);
  await page.getByRole("button", { name: "Cameras and media", exact: true }).click();
  await expect(page.getByRole("dialog", { name: "Cameras and media" })).toBeVisible();
  await page.keyboard.press("Escape");
  await expect(page.getByRole("dialog", { name: "Cameras and media" })).toBeHidden();
  await expect(page.locator("[data-video-analysis-fs-player-workstation]")).toHaveClass(/is-code-mode/);
});
