...
- A read-only local provider registry lets future detection, association, re-identification, team, ball, referee, and shirt stages report capability readiness without activating them. It hashes the actual sealed runtime and model files, rejects links/path escape/duplicate identities, reproduces the original report-evidence binding, and sends only bounded provider identity, capability, status, and reason codes to the browser. Registration never grants execution permission. A separate stage runner now requires one native sealed runtime, read-only content and descriptor seals for provider/model/source inputs before and after execution, a network-denied macOS sandbox, a scrubbed environment, session-owned source reuse, provider concurrency, timeout/output/process-group memory limits, cancellation, and exact request-bound result validation before `executionAvailable` can become true. Missing runtime isolation remains visibly `activation pending`.
- Benchmark collection cannot depend on prior approval. A separate candidate registry accepts only reviewed, offline, checksum-bound `candidate` installations and reuses the same sandbox in `benchmark-candidate` mode. Candidate registry metadata, job type, route, and browser result are all marked `benchmarkOnly`; the activated parser rejects the same manifest. The only generic activation path resolves one exact candidate plus its passed report and reproducible evidence, derives a new canonical approved manifest, atomically copies the same sealed runtime/models into the distinct approved registry, reruns registry and sandbox preflight, and never edits the candidate. Every candidate run also emits an immutable `football-science-tracking-candidate-stage-run-v1` artifact whose exact response SHA, canonical request/result, provider/build fingerprints, source/range, sandbox telemetry, and real-time factor are independently verified by the browser.
- Full-scene candidate execution composes detection, association, re-identification, and classification without weakening their provider boundaries. The legacy path accepts only detectors that explicitly distinguish player, ball, and referee. The role-aware path accepts generic person plus ball detection, then requires a separate `classify:role` result before any trajectory may reach player re-identification; unresolved people enter review as `unknown`, never silently as players. Non-unknown player/referee and home/away semantics require analyst-bound role and team anchors. Anchors are device-local, source- and association-artifact-bound, capped, conflict-checked, auditable, and invalidated before classification when association output changes. Every stage contributes evidence/result hashes to one lineage fingerprint, and the resulting player/ball/referee/unknown review tracks preserve role confidence. Position, visibility, continuity, split, identity swap, role/team/shirt, and player assignment corrections operate on review copies while raw candidate stage evidence remains recursively immutable. A source-, workspace-, case-, tenant-, user-, clip-, item-, and angle-bound local effort ledger separately counts exact review decisions, undo/defer actions, correction handoffs, and successful saved tracks. Restored campaign rows are revalidated against their sealed case source before their counts enter the UI; complete item/clip/angle context can seed a source-bound next-case handoff. After source matching, that handoff restores the exact presentation item, clip, and camera angle and validates them again before workspace selection or import. It survives picker cancellation, a wrong source, or a removed target and clears only after the exact case opens. Legacy coverage remains explicitly partial. A complete case can emit one strict metadata-only workload snapshot bound to its exact source, camera, range, reviewed handoff, locked ground-truth case, and suite SHA-256. Track selection and correction preserve the original snapshot; source, angle, range, or a new draft clear it. A separate locked refinement-burden record counts retained/excluded preannotations, added tracks, point provenance, deduplicated correction operations, and reviewed checkpoints. Both records are excluded from provider evaluation inputs and have no path to provider approval or ground-truth attestation.
...
| Tracking and dynamic telestration | Implemented in candidate branch | Prompt/keyframe UX, fail-honest selected-object/full-scene capability matrix, automatic cancellable full-range continuation, identity-safe seam merge, cumulative elapsed/ETA, confidence and occlusion gates, atomic split/identity-swap repair with undo, chunked reload-safe local workspaces, fail-visible idempotent metadata retry, track-bound graphics, secure local source reuse, and pinned resident SAM 2.1 installer/runtime with measured cold/warm telemetry |
| Tracking quality and provider governance | Implemented in candidate branch | In-product checksum-bound real-match workflow, selected-object and football-scene benchmarks, optimal frame assignment, unique non-overlapping evidence time, class/identity/fragment diagnostics, pinned TrackEval reference gates, non-overridable workstation-speed approval bound to one non-identifying execution profile, cancellable loopback evaluation, portable evidence sets, nested-strict stage contracts, immutable candidate-run evidence, and an evidence-lineaged detection/association/re-ID/classification review pipeline |
