...
function normalizeArchiveQuery(query = {}) {
...
async function readAnalysisRoomArchive(query, actor) {
  ...
  const params = normalizeArchiveQuery(query);
  let payload;
  if (params.p_date_from && params.p_date_to && params.p_date_from > params.p_date_to) {
    payload = { matches: [], hasMore: false };
  } else {
    let result;
    try {
      result = await dbRequest('/rpc/video_library_search', {
  ...
  return { ok: true, payload: { schema: 'footballscience-analysis-room-archive-search-v1', ...payload, nextCursor, checkedAt: new Date().toISOString() } };
}
