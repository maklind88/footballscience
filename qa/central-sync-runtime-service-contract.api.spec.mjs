...
test("central sync runtime flushes tombstone generations as server deletes", async () => {
  const key = "football-schedule-v1";
  const harness = createServiceHarness({
    syncResult: { ok: true, key, value: "", revision: 8 },
  });

  harness.rawValues.set(key, "{\"events\":[{\"id\":\"delete-me\"}]}");
  harness.service.queueCentralStateWrite(key, "", { removed: true });
  await harness.service.flushCentralStateWrites();
  ...
  expect(harness.syncCalls).toEqual([
    {
      key,
      value: "",
      options: { removed: true, automaticRetry: false, baseRevision: 7 },
    },
  ]);
  expect(harness.rawValues.has(key)).toBe(false);
  expect(harness.manifest.entries[key]).toMatchObject({
...
test("central sync runtime retries pending tombstones after reload", async () => {
  const key = "football-medical-team-v1";
  const harness = createServiceHarness({
    syncResult: { ok: true, key, value: "", revision: 8 },
  });
  ...
  expect(harness.syncCalls).toEqual([
    {
      key,
      value: "",
      options: { removed: true, automaticRetry: true, baseRevision: 7 },
    },
  ]);
  expect(harness.manifest.entries[key]).toMatchObject({
    pendingCentralSync: false,
...
test("central sync runtime clears tombstone metadata when a later set resurrects a key", () => {
  const key = "football-schedule-v1";
  const harness = createServiceHarness();
  harness.rawValues.set(key, "A");
  harness.service.queueCentralStateWrite(key, "", { removed: true });
