...
4. Check Platform Health history to see whether the failing signal is new, repeated, improving, or worsening.
5. Read the Traffic Snapshot to identify whether the incident is dominated by `/api/chat`, `/api/presence`, `/api/app-state`, auth/permission failures, or runtime `5xx`.
6. If live is broken, use the manual `Production Rollback` workflow with a known-good deployment URL or id.
7. After any rollback or hotfix, confirm `npm run release:postdeploy` and authenticated live smoke pass.
8. Comment on the incident issue with the action taken and close it only after the signal is green again.
