...
- The approved SAM 2.1 provider has a pinned manifest, checksum-verifying installer, exact upstream execution-tree verification, isolated runtime, capability preflight, forward/backward propagation, and bounded artifact validation. Provider `1.2.0` keeps one verified model resident for sequential jobs, creates a fresh video state per job, exposes bounded generation/cold/warm telemetry, and destroys the generation on cancellation, timeout, or protocol drift. Model assets are installed explicitly on the analyst device and are never bundled into the web deployment; dense samples remain local.
...
- Tracking installation, latency approval, and real-match quality evidence are independent gates. The pinned SAM 2.1 resident worker now proves model reuse and separate cold/warm telemetry, but its measured CPU-default warm factor is 7.819x real time and therefore not yet an elite workstation budget. Performance remains fail-visible until an optimized provider passes the warm reference gate without reducing representative real-match quality.
