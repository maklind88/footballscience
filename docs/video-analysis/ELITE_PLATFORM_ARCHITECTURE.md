...
- A read-only local provider registry lets future detection, association, re-identification, team, ball, referee, and shirt stages report capability readiness without activating them. It hashes the actual sealed runtime and model files, rejects links/path escape/duplicate identities, reproduces the original report-evidence binding, and sends only bounded provider identity, capability, status, and reason codes to the browser. Registration never grants execution permission, and the readiness matrix shows `activation pending` until a separately verified isolated stage runner exists.
