...
- `api`: `/api/platform-identity` returns the signed-in actor's server-owned platform profile, memberships, manageable scopes, and app-state fallback checkpoints.
