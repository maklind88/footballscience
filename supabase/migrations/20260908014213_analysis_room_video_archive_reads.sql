-- Analysis Room read model. No source records or browser grants are changed.
grant usage on schema app_private to service_role;
create index if not exists video_library_archive_cursor_idx
  on public.video_matches (organization_id, team_id, (coalesce(match_date, date '0001-01-01')) desc, id desc)
  where status = 'active';
create index if not exists video_library_archive_search_idx
...
create or replace function public.video_library_archive(
  p_organization_id text, p_team_id text, p_actor_id text,
  p_mode text default 'search', p_search text default '', p_type text default 'all',
...
begin
  if coalesce(p_organization_id, '') = '' or coalesce(p_team_id, '') = ''
    or p_mode is null or p_mode not in ('search', 'calendar') or p_type is null or p_type not in ('all', 'match', 'training')
  ...
  if p_mode = 'calendar' and (p_date_from is null or p_date_to is null or p_date_to < p_date_from or p_date_to - p_date_from > 30) then
    raise exception 'Calendar requires a bounded month' using errcode = '22023';
  end if;
  with filtered as not materialized (
    ...
    select m.* from filtered m where p_mode = 'search'
      and (p_cursor_id is null or (coalesce(m.match_date, date '0001-01-01'), m.id) < (p_cursor_date, p_cursor_id))
    ...
    select m.*, count(*) over (partition by match_date) as day_count,
      row_number() over (partition by match_date order by id desc) as day_row
    from filtered m where p_mode = 'calendar'
  ), month_preview as (
    select r.match_date, max(r.day_count) as day_count,
      jsonb_agg(app_private.video_library_match_json(m, p_actor_id) order by m.id desc) as matches
    from month_rows r join public.video_matches m on m.id = r.id
      and m.organization_id = p_organization_id and m.team_id = p_team_id
    where r.day_row <= 2 group by r.match_date
  )
  select case when p_mode = 'search' then jsonb_build_object(
    ...
    'days', coalesce((select jsonb_agg(jsonb_build_object('date', match_date, 'count', day_count, 'matches', matches) order by match_date) from month_preview), '[]'::jsonb),
    'linkedScheduleIds', coalesce((select jsonb_agg(distinct coalesce(metadata->>'scheduleEventId', metadata->>'schedule_event_id'))
      from public.video_matches where organization_id = p_organization_id and team_id = p_team_id and status = 'active'
        and match_date between p_date_from and p_date_to
        and nullif(coalesce(metadata->>'scheduleEventId', metadata->>'schedule_event_id'), '') is not null), '[]'::jsonb)
  ) end into result;
  return result;
end;
$$;
revoke all on function public.video_library_archive(text, text, text, text, text, text, date, date, date, uuid, integer) from public, anon, authenticated;
grant execute on function public.video_library_archive(text, text, text, text, text, text, date, date, date, uuid, integer) to service_role;
