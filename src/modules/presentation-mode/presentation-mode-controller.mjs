...
function normalizeLineupFormation(value = "") {
  return normalizePresentationLineupFormation(value);
}

function normalizeLineupAssignments(lineup = {}) {
  return normalizePresentationLineupAssignments(lineup);
}

function normalizeMatchSquadPlayerIds(value = []) {
  return normalizePresentationMatchSquadPlayerIds(value);
}
...
export function createPresentationModeController(dependencies = {}) {
  ...
  function updateSlideDropIndicator(event) {
    ...
    const targetKey = `${target.insertIndex}:${target.side}`;
    if (state.slideDropTarget === targetKey) {
      return target;
    }
    clearSlideDropIndicators();
    state.slideDropTarget = targetKey;
  ...
  function handleSlideDragOver(event) {
    ...
    const target = updateSlideDropIndicator(event);
    if (!target) {
      return;
    }
    event.preventDefault();
