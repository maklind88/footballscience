...
function assertReadableAdapter(storageAdapter) {
  if (!storageAdapter || typeof storageAdapter.read !== "function") {
    throw new TypeError("Home Chat adapter requires a readable storage adapter.");
  }
}
...
export function createHomeChatReadAdapter(storageAdapter, defaultOptions = {}) {
  ...
  return Object.freeze({
    ...
    async writeMessage() {
      throw new Error("Home Chat adapter is read-only until migration is explicitly enabled.");
    },
