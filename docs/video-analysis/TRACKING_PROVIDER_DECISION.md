...
SAM 2.1 remains the installed, approved selected-object propagation engine and safe manual-prompt fallback. Provider `1.2.0` can propagate 2-8 analyst-selected objects in one shared video state when every prompt uses the same frame and synchronized range. It keeps the verified model resident across sequential bounded jobs, while every job creates and resets its own video state. Cancellation, timeout, protocol drift, or an inconsistent result terminates that worker generation before another job can run. It still returns separate review tracks and is not treated as an automatic detector or a re-identification engine.
...
The installed `1.2.0` checks on 2026-08-26 are deliberately reported without hiding latency. The final CPU-default warm run loaded the resident worker in 2.467 seconds, including 1.760 seconds of model load, completed the first one-second fixture in 10.517 seconds, and completed the identical warm job in 7.819 seconds. That is a 1.345x cold-to-warm improvement, but still a 7.819 real-time factor against the reference maximum of 1. The synthetic result contained 6 points, 80% observed coverage, and 0.966 mean confidence. A same-provider MPS comparison produced denser synthetic output, 13 points at 96% coverage and 0.994 confidence, but its 24.959-second warm job was about 3.2x slower than CPU. macOS therefore defaults to CPU while a reviewed `FS_SAM2_DEVICE=mps` override remains available for comparison. Resident reuse and inference wiring are proven; workstation-speed approval and real-match football quality are not. Only the representative real-match benchmark can establish football quality.
