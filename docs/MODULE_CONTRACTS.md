...
- `qa`: schema, API payload safety, module boundary, workstation UX, permission matrix, tracking review/correction, atomic sample-preserving split and identity-swap undo, idempotent client-track creation, secure local provider and benchmark jobs, strict SoccerNet/MOT ground-truth import with rights provenance and temporal scene coverage, unique non-overlapping evidence duration, exact suite/report checksums, pinned TrackEval identity and internal/reference cross-validation, pitch calibration, metre confidence gates, multi-angle clock conversion, bounded local compositing, rendered-pixel proof, and Analysis Room loader contracts must prove metadata-only central storage, millisecond precision, no component database access, keyboard-ready coding, no fabricated movement continuity, and no local video path or artifact URL leakage.
