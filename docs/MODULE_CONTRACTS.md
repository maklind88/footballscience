...
- `api`: `GET /api/leaderboard?month=YYYY-MM&teamId=<platform-team-uuid>` reads a server-derived monthly snapshot. Its `roster: [{playerId, displayName, number, position}]` is rebuilt from the exact team's active `squad_roster_memberships` and `squad_players`; zero-point rows and award selection never use browser-cached Player Profiles identity. Award and reversal POST bodies carry the same optional `teamId`. Each event includes its safe staff display name and immutable participant snapshots in `awards: [{playerId, playerName, points, placement}]`, so Activity and player history use the same ledger truth. Award batches are atomic and idempotent; reversal idempotency binds the key to the normalized event/reason hash, writes compensating negative transactions, and never edits or deletes an existing point transaction. Completed months are read-only in UI, API actions, and SQL.
...
- `dataSafety`: this is a dedicated database-first module. It must not be added to generic `/api/app-state` persistence or workspace-access normalization; Home visibility comes from the immutable platform module contract and owns no Leaderboard data.
