...
The single instrumented follow-up [run 35949935172](https://github.com/maklind88/footballscience/actions/runs/35949935172) passed all seven jobs and all 13 runtime checks on `0e1f0693`, without changed deadlines or a retry loop. It verifies the current native build on WebView2 153.0.4234.48; the earlier failed runner used 152.0.4191.66. The 7,969 ms hanging-candidate preparation spent 7,156 ms in durable extraction and 2 ms acquiring locks. These measurements do not explain the earlier uninstrumented timeout. Preserve that residual stability hold; do not claim a functional fix, causal environment diagnosis or physical Windows readiness. Candidate A/B recommendations are unchanged. Controlled Windows preparation diagnostics precede the next packaged conflict-dialog acceptance phase.
