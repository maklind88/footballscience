...
FS Player's tracking sidebar can open one sealed case at a time from `annotation-pack.json`, `workspace.json`, the matching `*.track-map.json`, and `*.suggestions.mot.txt`. It rechecks the workspace checksum, both case-file hashes, exact normalized clip SHA-256, MOT geometry, and complete track-map lineage in the browser. Associated trajectories and raw unassociated detections enter one bounded review queue; only the current suggestion is previewed. Accept, reject, next, and undo remain local until `Save accepted` explicitly persists accepted tracks through the normal tracking repository. For signed-in tracking scopes, unresolved decisions and their undo order are serialized in an isolated IndexedDB draft bound to organization, team, user, source, clip, presentation item, angle, workspace SHA-256, and case; unsigned sessions remain explicitly session-only. A pending preview is excluded from verification, continuation, correction persistence, and graphic binding. `Save & correct` persists only the current suggestion as `review`, keeps it selected for the existing identity/box/visibility/structural correction tools, and lets the analyst continue the queue explicitly. Saved tracks remain `review`, never `verified` or ground truth.
