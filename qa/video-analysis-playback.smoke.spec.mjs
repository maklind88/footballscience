  ...
  await expect.poll(() => page.evaluate(() => {
    ...
    return Boolean(
      workspace
        ...
        && ruler
        && code.left < deck.left
        && Math.abs(timeline.left - code.left) < 8
...
test("Video Analysis Tag Panel creates a 15 second timeline tag from a code button", async ({ page }) => {
  ...
  expect(deckAfterMove.x).toBeLessThan(deckBeforeMove.x - 36);
  expect(deckAfterMove.y).toBeGreaterThan(deckBeforeMove.y + 20);
  expect(Math.abs(deckAfterMove.x - deckDuringMove.x)).toBeLessThanOrEqual(2);
  expect(Math.abs(deckAfterMove.y - deckDuringMove.y)).toBeLessThanOrEqual(2);
  const deckEastHandle = await page.locator('[data-video-analysis-code-pip="video"] .video-analysis-code-pip-resize--e').boundingBox();
  expect(deckEastHandle).toBeTruthy();
  await page.mouse.move(deckEastHandle.x + deckEastHandle.width / 2, deckEastHandle.y + deckEastHandle.height / 2);
  await page.mouse.down();
  await page.mouse.move(deckEastHandle.x + deckEastHandle.width / 2 + 48, deckEastHandle.y + deckEastHandle.height / 2);
  const deckDuringResize = await page.locator('[data-video-analysis-code-pip="video"]').boundingBox();
  await page.mouse.up();
  const deckAfterResize = await page.locator('[data-video-analysis-code-pip="video"]').boundingBox();
  expect(Math.abs(deckAfterResize.x - deckAfterMove.x)).toBeLessThanOrEqual(2);
  expect(deckAfterResize.width).toBeGreaterThan(deckAfterMove.width + 28);
  expect(Math.abs(deckAfterResize.x - deckDuringResize.x)).toBeLessThanOrEqual(2);
  expect(Math.abs(deckAfterResize.width - deckDuringResize.width)).toBeLessThanOrEqual(2);
  ...
  const timelineLayout = await page.evaluate(() => {
    ...
    return {
      ...
      videoRight: videoFrame?.right ?? 0,
      videoWidth: videoFrame?.width ?? 0,
    };
  });
  expect(timelineLayout.videoWidth).toBeGreaterThan(300);
  expect(Math.abs(timelineLayout.frameLeft - timelineLayout.codeLeft)).toBeLessThanOrEqual(3);
...
test("Video Analysis timeline uses h:mm:ss and scrubs video by dragging the red playhead", async ({ page }) => {
  ...
  const ruler = page.locator("[data-video-analysis-timeline-ruler]");
  ...
  const y = rulerBox.y + Math.min(10, rulerBox.height / 2);
