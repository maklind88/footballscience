export const platformRoles = Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance", "medical", "guest"]);
const staffRoles = Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance", "medical"]);
const managerRoles = Object.freeze(["admin", "club-admin", "team-admin", "coach"]);
const medicalAccessRoles = Object.freeze(["admin", "club-admin", "team-admin", "coach", "performance", "medical"]);
const medicalEditRoles = Object.freeze(["admin", "club-admin", "team-admin", "medical", "performance"]);
const transferRoomAdminRoles = Object.freeze(["admin", "team-admin"]);

export const protectedStorageKeys = Object.freeze([
  "football-workspace-hub-v3",
  "football-platform-structure-v1",
  "football-platform-appearance-v1",
  "football-periodization-v2",
  "football-schedule-v1",
  "football-gameplan-v1",
  "football-set-pieces-room-v1",
  "football-session-planner-v3",
  "football-session-exercise-library-v1",
  "football-session-exercise-library-backup-v1",
  "football-session-exercise-library-folders-v1",
  "football-session-exercise-library-folders-backup-v1",
  "football-dashboard-tasks-v1",
  "football-dashboard-chat-v1",
  "football-dashboard-notification-seen-v1",
  "football-dashboard-tutorial-prefs-v1",
  "football-dashboard-news-seen-v1",
  "football-dashboard-presentation-mode-v1",
  "football-medical-team-v1",
  "football-player-profiles-v1",
  "football-scouting-v1",
  "football-transfer-room-v1",
  "football-simulator-sequence-v1",
  "football-simulator-sequence-library-v2",
]);

export const platformModuleMigrationOrder = Object.freeze([
  "platform-identity",
  "home",
  "chat",
  "platform-appearance",
  "schedule",
  "gameplan",
  "set-pieces-room",
  "video-analysis",
  "idp",
  "leaderboard",
  "exercise-library",
  "session-planner",
  "periodization",
  "medical-team",
  "football-science-db",
  "scouting",
  "transfer-room",
  "game-simulator",
]);

export const platformModules = Object.freeze([
  Object.freeze({
    id: "platform-shell",
    label: "Platform Shell",
    stage: "core",
    storageKeys: Object.freeze(["football-workspace-hub-v3", "football-platform-structure-v1"]),
    futureTables: Object.freeze(["clubs", "teams", "memberships"]),
    viewRoles: Object.freeze(platformRoles),
    editRoles: Object.freeze(["admin"]),
    emits: Object.freeze(["workspace.opened", "profile.opened", "auth.signed-out"]),
    consumes: Object.freeze(["data-safety.changed"]),
  }),
  Object.freeze({
    id: "platform-readiness",
    label: "Platform Readiness",
    stage: "core",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze(["platform_release_checks", "platform_observability_signals"]),
    viewRoles: Object.freeze(["admin"]),
    editRoles: Object.freeze(["admin"]),
    emits: Object.freeze(["platform.readiness-viewed", "platform.readiness-refreshed"]),
    consumes: Object.freeze(["release.status", "backup.status", "data-safety.changed", "permission.changed"]),
  }),
  Object.freeze({
    id: "platform-appearance",
    label: "Platform Appearance",
    stage: "core",
    storageKeys: Object.freeze(["football-platform-appearance-v1"]),
    futureTables: Object.freeze(["platform_appearance_configs", "platform_appearance_versions"]),
    viewRoles: Object.freeze(["admin"]),
    editRoles: Object.freeze(["admin"]),
    emits: Object.freeze(["appearance.previewed", "appearance.published", "appearance.rolled-back"]),
    consumes: Object.freeze(["data-safety.changed", "permission.changed"]),
  }),
  Object.freeze({
    id: "platform-identity",
    label: "Platform Identity",
    stage: "core",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze([
      "platform_organizations",
      "platform_clubs",
      "platform_teams",
      "platform_user_profiles",
      "platform_memberships",
      "platform_tenant_links",
    ]),
    viewRoles: Object.freeze(platformRoles),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin"]),
    emits: Object.freeze(["tenant.created", "membership.changed", "scope.changed"]),
    consumes: Object.freeze(["auth.signed-in", "profile.updated", "permission.changed"]),
  }),
  Object.freeze({
    id: "home",
    label: "Home",
    stage: "module",
    storageKeys: Object.freeze([
      "football-dashboard-tasks-v1",
      "football-dashboard-notification-seen-v1",
      "football-dashboard-tutorial-prefs-v1",
      "football-dashboard-news-seen-v1",
      "football-dashboard-presentation-mode-v1",
    ]),
    futureTables: Object.freeze(["tasks"]),
    viewRoles: Object.freeze(platformRoles),
    editRoles: Object.freeze(staffRoles),
    emits: Object.freeze(["task.created", "task.completed"]),
    consumes: Object.freeze(["chat.unread-count.changed"]),
  }),
  Object.freeze({
    id: "chat",
    label: "Team Chat",
    stage: "module",
    storageKeys: Object.freeze(["football-dashboard-chat-v1"]),
    futureTables: Object.freeze([
      "chat_organizations",
      "chat_teams",
      "chat_team_memberships",
      "chat_threads",
      "chat_thread_participants",
      "chat_messages",
      "chat_message_mentions",
      "chat_reactions",
      "chat_read_receipts",
      "chat_attachments",
      "chat_audit_events",
      "chat_retention_policies",
    ]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(staffRoles),
    emits: Object.freeze(["chat.message-sent", "chat.message-read"]),
    consumes: Object.freeze(["profile.updated"]),
  }),
  Object.freeze({
    id: "schedule",
    label: "Schedule",
    stage: "module",
    storageKeys: Object.freeze(["football-schedule-v1"]),
    futureTables: Object.freeze(["schedule_events"]),
    viewRoles: Object.freeze(platformRoles),
    editRoles: Object.freeze(managerRoles),
    emits: Object.freeze(["schedule.event-created", "schedule.event-updated", "schedule.date-selected"]),
    consumes: Object.freeze([]),
  }),
  Object.freeze({
    id: "gameplan",
    label: "Gameplan",
    stage: "module",
    storageKeys: Object.freeze(["football-gameplan-v1"]),
    futureTables: Object.freeze([
      "gameplan_match_plans",
      "gameplan_staff_roles",
      "gameplan_player_briefs",
      "gameplan_scenarios",
      "gameplan_evidence",
      "gameplan_observations",
    ]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst"]),
    emits: Object.freeze(["gameplan.updated", "gameplan.player-brief-published"]),
    consumes: Object.freeze(["schedule.event-updated", "profile.updated"]),
  }),
  Object.freeze({
    id: "set-pieces-room",
    label: "Set Pieces Room",
    stage: "module",
    storageKeys: Object.freeze(["football-set-pieces-room-v1"]),
    futureTables: Object.freeze([
      "set_piece_plans",
      "set_piece_variants",
      "set_piece_phases",
      "set_piece_elements",
      "set_piece_drawings",
      "set_piece_versions",
    ]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "analyst"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "analyst"]),
    emits: Object.freeze(["set-piece.updated", "set-piece.variant-created", "set-piece.played"]),
    consumes: Object.freeze(["profile.updated", "schedule.event-updated"]),
  }),
  Object.freeze({
    id: "video-analysis",
    label: "FS Player",
    stage: "module",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze([
      "video_matches",
      "video_videos",
      "video_sources",
      "video_clip_instances",
      "video_clip_players",
      "video_clip_tags",
      "video_clip_notes",
      "video_coding_schemas",
      "video_playlists",
      "video_playlist_items",
      "video_audit_events",
    ]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "analyst"]),
    emits: Object.freeze(["video.local-source-linked", "video.clip-saved", "video.review-list-updated"]),
    consumes: Object.freeze(["schedule.event-updated", "profile.updated", "gameplan.updated"]),
  }),
  Object.freeze({
    id: "idp",
    label: "IDP",
    stage: "module",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze([
      "idp_profiles",
      "idp_development_areas",
      "idp_focuses",
      "idp_clip_bank_items",
      "idp_evidence",
      "idp_reviews",
      "idp_next_actions",
      "idp_milestones",
      "idp_staff_ownership",
      "idp_audit_events",
    ]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance", "medical"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "analyst"]),
    emits: Object.freeze(["idp.focus-updated", "idp.evidence-added", "idp.review-completed"]),
    consumes: Object.freeze(["profile.updated", "video.clip-saved", "video.player-tagged"]),
  }),
  Object.freeze({
    id: "exercise-library",
    label: "Exercise Library",
    stage: "module",
    storageKeys: Object.freeze([
      "football-session-exercise-library-v1",
      "football-session-exercise-library-backup-v1",
      "football-session-exercise-library-folders-v1",
      "football-session-exercise-library-folders-backup-v1",
    ]),
    futureTables: Object.freeze(["exercises", "exercise_versions", "exercise_folders", "exercise_folder_items"]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(managerRoles),
    emits: Object.freeze(["exercise.saved", "exercise.archived", "exercise.restored", "exercise.folder-updated"]),
    consumes: Object.freeze(["session.block-updated"]),
  }),
  Object.freeze({
    id: "session-planner",
    label: "Session Planner",
    stage: "module",
    storageKeys: Object.freeze(["football-session-planner-v3"]),
    futureTables: Object.freeze(["sessions", "session_blocks"]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(managerRoles),
    emits: Object.freeze(["session.block-updated", "session.date-selected"]),
    consumes: Object.freeze(["schedule.date-selected", "periodization.day-updated", "medical.availability-updated"]),
  }),
  Object.freeze({
    id: "periodization",
    label: "Periodization",
    stage: "module",
    storageKeys: Object.freeze(["football-periodization-v2"]),
    futureTables: Object.freeze(["periodization_days"]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "performance"]),
    emits: Object.freeze(["periodization.day-updated"]),
    consumes: Object.freeze(["schedule.event-updated"]),
  }),
  Object.freeze({
    id: "medical-team",
    label: "Medical Team",
    stage: "module",
    storageKeys: Object.freeze(["football-medical-team-v1"]),
    futureTables: Object.freeze(["medical_availability", "medical_injury_plans"]),
    viewRoles: Object.freeze(medicalAccessRoles),
    editRoles: Object.freeze(medicalEditRoles),
    emits: Object.freeze(["medical.availability-updated"]),
    consumes: Object.freeze(["profile.updated"]),
  }),
  Object.freeze({
    id: "player-profiles",
    label: "Squad",
    stage: "module",
    storageKeys: Object.freeze(["football-player-profiles-v1"]),
    futureTables: Object.freeze([
      "squad_organizations",
      "squad_clubs",
      "squad_teams",
      "squad_seasons",
      "squad_players",
      "squad_roster_memberships",
    ]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "performance", "medical"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout"]),
    emits: Object.freeze(["profile.updated"]),
    consumes: Object.freeze(["medical.availability-updated"]),
  }),
  Object.freeze({
    id: "leaderboard",
    label: "Leaderboard",
    stage: "module",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze([
      "leaderboard_competitions",
      "leaderboard_participants",
      "leaderboard_scoring_events",
      "leaderboard_point_transactions",
      "leaderboard_audit_events",
    ]),
    viewRoles: Object.freeze(staffRoles),
    editRoles: Object.freeze(managerRoles),
    emits: Object.freeze([
      "leaderboard.points-awarded",
      "leaderboard.batch-reversed",
      "leaderboard.competition-closed",
    ]),
    consumes: Object.freeze(["profile.updated", "schedule.event-updated"]),
  }),
  Object.freeze({
    id: "football-science-db",
    label: "Football Science DB",
    stage: "foundation",
    storageKeys: Object.freeze([]),
    futureTables: Object.freeze([
      "fsdb_players",
      "fsdb_player_aliases",
      "fsdb_player_source_links",
      "fsdb_teams",
      "fsdb_competitions",
      "fsdb_roster_entries",
      "fsdb_player_season_stats",
      "fsdb_import_batches",
      "fsdb_import_errors",
    ]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance", "medical"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "scout", "analyst"]),
    emits: Object.freeze(["fsdb.player-imported", "fsdb.identity-linked", "fsdb.roster-updated"]),
    consumes: Object.freeze(["profile.updated", "scouting.import-published"]),
  }),
  Object.freeze({
    id: "scouting",
    label: "Scouting",
    stage: "module",
    storageKeys: Object.freeze(["football-scouting-v1"]),
    futureTables: Object.freeze(["scouting_targets", "scouting_reports", "scouting_shortlists"]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst"]),
    emits: Object.freeze(["scouting.target-created", "scouting.report-created", "scouting.shortlist-updated"]),
    consumes: Object.freeze(["profile.updated", "schedule.event-updated"]),
  }),
  Object.freeze({
    id: "transfer-room",
    label: "Transfer Room",
    stage: "module",
    storageKeys: Object.freeze(["football-transfer-room-v1"]),
    futureTables: Object.freeze([
      "transfer_room_access_grants",
      "transfer_room_rule_profiles",
      "transfer_room_squad_plans",
      "transfer_room_target_plans",
      "transfer_room_window_plans",
      "transfer_room_decision_events",
    ]),
    viewRoles: transferRoomAdminRoles,
    editRoles: transferRoomAdminRoles,
    emits: Object.freeze(["transfer-room.plan-updated", "transfer-room.target-added", "transfer-room.access-updated"]),
    consumes: Object.freeze(["profile.updated", "scouting.shortlist-updated", "scouting.target-created"]),
  }),
  Object.freeze({
    id: "game-simulator",
    label: "Game Simulator",
    stage: "module",
    storageKeys: Object.freeze(["football-simulator-sequence-v1", "football-simulator-sequence-library-v2"]),
    futureTables: Object.freeze(["simulator_sequences", "simulator_sequence_versions"]),
    viewRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst", "performance"]),
    editRoles: Object.freeze(["admin", "club-admin", "team-admin", "coach", "scout", "analyst"]),
    emits: Object.freeze(["simulator.sequence-saved", "simulator.sequence-loaded"]),
    consumes: Object.freeze([]),
  }),
]);

export const platformModuleIds = Object.freeze(platformModules.map((module) => module.id));

export function getProtectedStorageKeySet() {
  return new Set(protectedStorageKeys);
}
