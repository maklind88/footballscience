...
export const moduleStandardContracts = Object.freeze([
  ...
  createModuleStandardContract({
    ...
    currentFiles: [
      ...
      "docs/SESSION_PLANNER_DOMAIN_RECORDS_V1.md",
    ],
    ...
    testFiles: [
      ...
      "qa/session-planner-domain-records.api.spec.mjs",
      "qa/session-planner-domain-schema.api.spec.mjs",
      "qa/session-planner-database.api.spec.mjs",
      "qa/session-planner-migration-safety.api.spec.mjs",
      "qa/session-planner-postgres-drill.api.spec.mjs",
    ],
    extractionOrder: 12,
    riskLevel: "high",
    notes: "Frontend rendering and controllers are partially extracted. The additive Session Planner domain-record pilot owns bounded session/block records, scoped shadow reads, migration safety, and rollback contracts while app-state remains production-primary; normal application database writes stay disabled.",
  }),
  createModuleStandardContract({
    id: "gameplan",
    label: "Gameplan",
