...
- Full-scene candidate execution composes detection, association, re-identification, and classification without weakening their provider boundaries. Complete detection observations are materialized into ordered trajectories before identity stages, every stage contributes evidence/result hashes to one lineage fingerprint, and the resulting player/ball/referee tracks enter normal review. Position, visibility, continuity, split, identity swap, team/shirt, and player assignment corrections operate on review copies while raw candidate stage evidence remains recursively immutable. A source-, workspace-, case-, tenant-, user-, clip-, item-, and angle-bound local effort ledger separately counts exact review decisions, undo/defer actions, correction handoffs, and successful saved tracks. Legacy coverage remains explicitly partial. A complete case can emit one strict metadata-only workload snapshot bound to its exact source, camera, range, reviewed handoff, locked ground-truth case, and suite SHA-256. Track selection and correction preserve the original snapshot; source, angle, range, or a new draft clear it. A separate locked refinement-burden record counts retained/excluded preannotations, added tracks, point provenance, deduplicated correction operations, and reviewed checkpoints. Both records are excluded from provider evaluation inputs and have no path to provider approval or ground-truth attestation.
