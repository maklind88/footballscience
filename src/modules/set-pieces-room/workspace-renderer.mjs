...
function renderToolRail(ui = {}, roster = []) {
  return `<div class="spr-tool-rail" role="toolbar" aria-label="Tactical tools">
    ${setPieceToolOptions.map((tool) => `<button type="button" class="spr-tool-button ${ui.activeTool === tool.value ? "is-active" : ""}" data-set-piece-tool="${tool.value}" aria-label="${escapeSetPieceHtml(tool.label)}" title="${escapeSetPieceHtml(`${tool.label} (${tool.shortcut})`)}" ${tool.value === "home-player" && !roster.length ? "disabled" : ""}><span aria-hidden="true">${tool.symbol}</span></button>`).join("")}
  </div>`;
}
...
function renderPlayLibrary(state = {}, ui = {}) {
  ...
  return `<aside class="spr-library" aria-label="Set piece library">
    ...
    <div class="spr-segmented" role="group" aria-label="Library filter">
      ${[{ value: "all", label: "All" }, ...setPieceMomentOptions].map((option) => `<button type="button" data-set-piece-filter="${option.value}" class="${ui.libraryFilter === option.value ? "is-active" : ""}">${option.label.replace("ing", "")}</button>`).join("")}
    </div>
...
function renderBoardWorkspace(play, variant, phase, roster, ui, canEdit) {
  ...
  return `<main class="spr-editor">
    ...
    <div class="spr-editor-toolbar">
      <label class="spr-roster-picker"><span>Own player</span><select data-set-piece-roster-select ${roster.length && canEdit ? "" : "disabled"}>${roster.map((entry) => `<option value="${escapeSetPieceHtml(entry.id)}" ${entry.id === ui.selectedRosterId ? "selected" : ""}>${escapeSetPieceHtml(entry.name)}</option>`).join("") || '<option value="">No squad profiles</option>'}</select></label>
    ...
    <div class="spr-playback" aria-label="Playback controls">
      ...
      <span class="spr-phase-counter">${phaseIndex + 1} / ${variant.phases.length}</span>
    ...
    <div class="spr-timeline" aria-label="Phase timeline">
      <div class="spr-phase-strip">
        ${variant.phases.map((item, index) => `<button type="button" class="spr-phase-card ${item.id === phase.id ? "is-active" : ""}" data-set-piece-phase-id="${escapeSetPieceHtml(item.id)}">
          ...
          <span><b>${String(index + 1).padStart(2, "0")}</b>${escapeSetPieceHtml(item.title)}</span>
        </button>`).join("")}
...
function renderPlanInspector(play, variant, phase, ui, canEdit) {
  return `<div class="spr-inspector-section">
    <div class="spr-inspector-title"><div><p>Plan</p><strong>${escapeSetPieceHtml(play.title)}</strong></div><button type="button" class="spr-icon-button" data-set-piece-action="duplicate-play" title="Duplicate plan" aria-label="Duplicate plan" ${canEdit ? "" : "disabled"}>⧉</button></div>
  ...
  <div class="spr-inspector-section">
    <div class="spr-inspector-title"><div><p>Variant</p><strong>${escapeSetPieceHtml(variant.title)}</strong></div>${play.variants.length > 1 ? `<button type="button" class="spr-icon-button is-danger" data-set-piece-action="delete-variant" title="Delete variant" aria-label="Delete variant" ${canEdit ? "" : "disabled"}>⌫</button>` : ""}</div>
...
export function renderSetPiecesWorkspace(options = {}) {
  ...
  return `<section class="spr-shell ${ui.presentationMode ? "is-presenting" : ""}" data-set-pieces-room>
