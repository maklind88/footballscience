...
- `id`: `leaderboard`
- `purpose`: one team-scoped points competition per calendar month, with dated training awards, SQL-derived standings, tie ranks using `rank()` semantics, point distribution, and auditable corrections.
- `data`: database-primary records in `leaderboard_competitions`, `leaderboard_participants`, `leaderboard_scoring_events`, `leaderboard_point_transactions`, and `leaderboard_audit_events`. Totals and rank are derived from the append-only point ledger; there is no mutable total column and no browser storage source of truth.
- `api`: `GET /api/leaderboard?month=YYYY-MM&teamId=<platform-team-uuid>` reads a server-derived monthly snapshot. Its `roster: [{playerId, displayName, number, position, availabilityStatus, availabilityByDate}]` is rebuilt from the exact team's active `squad_roster_memberships` and `squad_players`; while Squad's migration checkpoint keeps app-state fallback enabled, an exact-tenant, revision/hash-idempotent server projection refreshes those canonical rows from `football-player-profiles-v1`. Zero-point rows and award selection never use browser-cached Player Profiles identity. `availabilityByDate` consumes Medical's coach-safe contract and contains only status, participation percentage, eligibility, and provenance—never diagnosis, body area, comments, notes, clearance detail, or injury-plan content. Award and reversal POST bodies carry the same optional `teamId`. Each event includes its safe staff display name and immutable participant snapshots in `awards: [{playerId, playerName, points, placement}]`, so Activity and player history use the same ledger truth. Award batches are atomic and idempotent; unavailable players are rejected for the award date; reversal idempotency binds the key to the normalized event/reason hash, writes compensating negative transactions, and never edits or deletes an existing point transaction. Completed months are read-only in UI, API actions, and SQL.
- `identity`: platform organization/team scope comes only from fresh active `platform_memberships`; an explicit `teamId` selects the active workspace team but grants no authority and is freshly checked against direct, club, or organization membership. Omission falls back only when exactly one active direct team membership is unambiguous. Every read and write RPC rechecks the actor, tenant, team, and allowed membership role at execution time. Bootstrap/user metadata and display names cannot elevate or select that role. A legacy Squad player id is accepted only when the server uniquely maps it to an active `squad_players.metadata.legacyId` and active team roster membership. Missing or ambiguous team/player links fail closed.
- `permissions`: active staff team members can read according to the permission matrix; Guest is excluded. Platform Admin, Club Admin, Team Admin, and Coach can award or reverse points. There is no public hard-delete operation.
- `permissionMatrix`: `leaderboard` and `/api/leaderboard` must remain registered in `src/core/permission-matrix.cjs` and seeded into `public.platform_permission_matrix`.
- `surface`: Leaderboard has no standalone workspace/navigation entry. Home mounts the current-month summary and the module-owned full-screen dialog through `mountLeaderboardHome`; Session Planner can request the same surface only through `openLeaderboardAward({ occurredOn, title }, opener)`. Scope changes force-dispose the previous runtime and abort stale reads, while ordinary Home navigation cannot tear down a pending reversal.
- `qa`: schema contracts must prove composite tenant foreign keys, RLS, revoked client grants, service-role-only RPC execution, append-only transaction/audit guards, SQL `rank()`, and idempotency. API contracts must prove server-owned actor scope, cross-team denial, fail-closed Squad mapping, request validation, and stable response shape.
- `dataSafety`: this is a dedicated database-first module. It must not be added to generic `/api/app-state` persistence or workspace-access normalization; Home visibility comes from the immutable platform module contract and owns no Leaderboard data. The temporary Squad projection is one-way and server-owned: it never writes to Player Profiles or Medical state, is locked to the verified legacy tenant, soft-deactivates only projection-owned memberships, records an import batch and audit event, and stops being needed when Squad becomes database-primary.
...
- `data`: database-primary metadata in `video_matches`, `video_videos`, `video_sources`, `video_clip_instances`, `video_clip_players`, `video_clip_tags`, `video_clip_notes`, `video_coding_schemas`, `video_playlists`, `video_playlist_items`, `video_coding_templates`, `video_coding_buttons`, `video_coding_button_links`, `video_clip_labels`, `video_clip_descriptors`, `video_timeline_lanes`, `video_saved_clip_searches`, `video_playlist_sections`, `video_review_sessions`, `video_clip_revisions`, `video_object_tracks`, `video_track_corrections`, `video_dynamic_graphics`, `video_pitch_calibrations`, `video_pitch_calibration_frames`, `video_media_angles`, and `video_export_manifests`; dense tracking samples, locked ground truth, raw provider runs, and pending metadata-only correction audits use bounded, exact-scope IndexedDB workspaces. Benchmark evaluation is bounded and local, while its checksum-bound evidence set leaves memory only through explicit device download. Raw media, rendered media, device paths, and artifact URLs remain local, with no protected browser storage key and no implicit cloud video uploads.
...
- `events`: local source linked, camera angle linked, angle synchronization changed, code button selected, clip saved, search saved, review section updated, local export completed.
- `qa`: schema, API payload safety, module boundary, workstation UX, permission matrix, tracking review/correction, atomic sample-preserving split and identity-swap undo, idempotent client-track creation, secure local provider and benchmark jobs, atomic hash-bound benchmark-candidate installation with explicit licence acceptance and native sandbox preflight, immutable five-case candidate screening with independent raw-evidence reproduction, strict SoccerNet/MOT ground-truth import with rights provenance and temporal scene coverage, unique non-overlapping evidence duration, exact suite/report checksums, pinned TrackEval identity and internal/reference cross-validation, pitch calibration, metre confidence gates, multi-angle clock conversion, bounded local compositing, rendered-pixel proof, and Analysis Room loader contracts must prove metadata-only central storage, millisecond precision, no component database access, keyboard-ready coding, no fabricated movement continuity, and no local video path or artifact URL leakage.
...
- `migration`: move through the read-only Squad adapter first, then dual-read / dual-write into `squad_organizations`, `squad_clubs`, `squad_teams`, `squad_seasons`, `squad_players`, `squad_roster_memberships`, and supporting `squad_*` history/import/media tables. Preserve `football-player-profiles-v1` until database reads, backups, and rollback drills are verified. Leaderboard may consume the temporary exact-tenant `sync_squad_roster_projection` contract, but does not own Player Profiles writes; the projection is revision/hash-idempotent, audited, and limited to ordinary active roster players.
