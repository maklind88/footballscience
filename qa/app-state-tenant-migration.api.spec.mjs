...
function extractSqlFunctions(sql) {
  return [...sql.matchAll(/create or replace function public\.([a-z0-9_]+)\([\s\S]*?\n\)\nreturns table \([\s\S]*?\n\)\nlanguage plpgsql\nsecurity invoker\nset search_path = public, pg_temp\nas \$\$([\s\S]*?)\n\$\$;/gi)]
    .map((match) => ({ name: match[1], body: match[2], source: match[0] }));
}

function expectBalancedParentheses(body) {
  let depth = 0;
  let inString = false;
...
test("migration SQL has complete parseable function bodies and service-role grants", async () => {
  const sql = await readFile(migrationSqlUrl, "utf8");
  const functions = extractSqlFunctions(sql);

  expect((sql.match(/\$\$/g) || [])).toHaveLength(4);
  expect(functions.map((entry) => entry.name).sort()).toEqual([
    "apply_platform_app_state_tenant_migration",
    "rollback_platform_app_state_tenant_migration",
  ]);
  for (const entry of functions) {
    expectBalancedParentheses(entry.body);
    expect(entry.source).toContain("jsonb_to_recordset(coalesce(p_source_rows, '[]'::jsonb)) as");
    expect(entry.source).toContain("metadata jsonb");
    expect(entry.source).toContain("security invoker");
    expect(entry.source).toContain("set search_path = public, pg_temp");
  }
  expect(sql).toContain("lock table public.platform_app_state_records in share row exclusive mode");
  expect(sql).not.toContain("on conflict (organization_id, state_key) do nothing");
  expect(sql).not.toContain("on conflict on constraint platform_app_state_records_pkey do nothing");
  expect(sql).toContain("get diagnostics updated_journal_count = row_count");
  expect(sql).toContain("p_expected_journal_status");
  expect(sql).toContain("p_expected_source_content_sha256");
  expect(sql).toContain("p_expected_source_revision_sha256");
  expect(sql).toContain("revoke all on function public.apply_platform_app_state_tenant_migration");
  expect(sql).toContain("grant execute on function public.apply_platform_app_state_tenant_migration");
  expect(sql).toContain("revoke all on function public.rollback_platform_app_state_tenant_migration");
  expect(sql).toContain("grant execute on function public.rollback_platform_app_state_tenant_migration");
});
...
test("apply uses the transactional migration RPC and fails closed on completed-journal errors", async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/rest/v1/platform_app_state_tenant_migrations" && method === "PATCH") {
      ...
      return new Response(JSON.stringify([{
        id: migrationId,
        status: requests.at(-1).body?.status,
        plan_sha256: plan.planSha256,
      }]), { status: 200 });
    }
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/rest/v1/rpc/rollback_platform_app_state_tenant_migration") {
      expect(requests.at(-1).body).toMatchObject({
        ...
        p_expected_source_content_sha256: plan.source.contentSha256,
        p_expected_source_revision_sha256: plan.source.revisionSha256,
        p_expected_journal_status: "completed",
      });
      targetRows = [];
...
test("apply fails closed when transactional RPC reports a partial migration", async () => {
  const sourceRows = [row()];
  const plan = createAppStateTenantMigrationPlan({ targetOrganizationId, sourceRows, targetRows: [] });
  const migrationId = "00000000-0000-4000-8000-000000000403";
  const snapshot = createMigrationSnapshot(plan);
  const patchBodies = [];
  let uploadedSnapshot = null;
  const fetchImpl = async (url, options = {}) => {
    ...
    if (requestUrl.pathname === "/rest/v1/platform_app_state_records") {
      return new Response(JSON.stringify(requestUrl.searchParams.get("organization_id") === "eq.global" ? sourceRows : []), { status: 200 });
    }
    if (requestUrl.pathname === "/rest/v1/platform_app_state_tenant_migrations" && method === "GET") {
      return new Response(JSON.stringify([]), { status: 200 });
    }
    ...
    if (requestUrl.pathname.startsWith("/storage/v1/object/footballscience-app-state/")) {
      if (method === "POST") {
        uploadedSnapshot = body;
        return new Response(JSON.stringify({ ok: true }), { status: 200 });
      }
      return new Response(JSON.stringify(uploadedSnapshot || snapshot), { status: 200 });
    }
    if (requestUrl.pathname === "/rest/v1/platform_app_state_tenant_migrations" && method === "POST") {
      return new Response(JSON.stringify([{ id: migrationId, ...body }]), { status: 201 });
    }
    if (requestUrl.pathname === "/rest/v1/rpc/apply_platform_app_state_tenant_migration") {
      return new Response(JSON.stringify([{ applied: true, migrated_count: 0 }]), { status: 200 });
    }
    if (requestUrl.pathname === "/rest/v1/platform_app_state_tenant_migrations" && method === "PATCH") {
      patchBodies.push(body);
      expect(requestUrl.searchParams.get("status")).toBe("eq.applying");
      expect(requestUrl.searchParams.get("plan_sha256")).toBe("eq." + plan.planSha256);
      return new Response(JSON.stringify([{ id: migrationId, status: body.status, plan_sha256: plan.planSha256 }]), { status: 200 });
    }
    return new Response(JSON.stringify({ message: `Unexpected request: ${requestUrl.pathname}` }), { status: 500 });
  };
  ...
  await expect(runAppStateTenantMigration({
    ...
    config: { url: "https://example.supabase.co", serviceRoleKey: "service-role-test-key" },
    fetchImpl,
  })).rejects.toThrow("Legacy app-state rows could not be migrated transactionally");

  expect(patchBodies.map((body) => body.status)).toEqual(["failed"]);
});

test("journal CAS update that affects no rows is a hard failure", async () => {
  const sourceRows = [row()];
  let targetRows = [];
  const plan = createAppStateTenantMigrationPlan({ targetOrganizationId, sourceRows, targetRows });
  const snapshot = createMigrationSnapshot(plan);
  const migrationId = "00000000-0000-4000-8000-000000000404";
  let uploadedSnapshot = null;
  const fetchImpl = async (url, options = {}) => {
    const requestUrl = new URL(String(url));
    const method = String(options.method || "GET").toUpperCase();
    ...
    if (requestUrl.pathname === "/rest/v1/platform_app_state_records") {
      return new Response(JSON.stringify(requestUrl.searchParams.get("organization_id") === "eq.global" ? sourceRows : targetRows), { status: 200 });
    }
    if (requestUrl.pathname === "/rest/v1/platform_app_state_tenant_migrations" && method === "GET") {
      return new Response(JSON.stringify([]), { status: 200 });
    }
  ...
  await expect(runAppStateTenantMigration({
    ...
    config: { url: "https://example.supabase.co", serviceRoleKey: "service-role-test-key" },
    fetchImpl,
  })).rejects.toThrow("Migration journal CAS update affected no rows");
});
