...
The in-product preannotation queue keeps that boundary intact. The analyst selects the pack manifest, workspace manifest, and one matching track-map/MOT pair after reconnecting the exact normalized case clip. Browser-side SHA-256, workspace checksum, source, geometry, count, and lineage verification runs before review begins. The normal tracking overlay renders only one pending suggestion at a time, prioritizing referee and ball work before player trajectories. Accept and reject decisions are reversible and device-session local; only `Save accepted` persists accepted tracks, and those tracks remain in review status. Changing clips clears an unsaved preview rather than carrying evidence across sources.
