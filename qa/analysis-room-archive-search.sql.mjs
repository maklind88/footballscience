...
if (!packageRoot) throw new Error('Pass the local @electric-sql/pglite package directory. See docs/analysis-room/ARCHIVE_SEARCH.md.');
...
try {
  ...
  await db.exec(await readFile(new URL('supabase/migrations/20260910012100_analysis_room_archive_search.sql', root), 'utf8'));
  await db.exec(`
  ...
  const query = async (overrides = {}) => {
    const q = { org: 'club-a', team: 'team-a', search: '', type: 'all', from: null, to: null, cursorDate: null, cursorId: null, limit: 8, ...overrides };
    const started = performance.now();
    const result = await db.query('select public.video_library_search($1,$2,$3,$4,$5,$6::date,$7::date,$8::date,$9::uuid,$10) as result',
      [q.org,q.team,'coach-a',q.search,q.type,q.from,q.to,q.cursorDate,q.cursorId,q.limit]);
    console.log(`search: ${Math.round(performance.now() - started)} ms, ${JSON.stringify(result.rows[0].result).length} bytes`);
  ...
  assert.equal((await query({ type: 'match' })).matches.length, 0);
  assert.equal((await query({ from: '2020-01-02', to: '2020-01-02', search: 'Historic' })).matches.length, 1);
  ...
  assert.match(JSON.stringify(plan.rows), /video_library_search_cursor_idx/);
  console.log('SQL proof passed: 50,000 sessions + 50,000 clips, keyset pages, literal search, exact counts and role/tenant isolation.');
} finally {
  await db.close();
}
